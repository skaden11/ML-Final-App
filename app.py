...
def load_artifacts():
    ...
    if missing:
        raise FileNotFoundError(
            ...
            + ", ".join(missing)
            + ". On Streamlit Cloud, make sure the GitHub repo contains an `artifacts/` "
...
def load_test_predictions():
    ...
    if not predictions_path.exists():
        raise FileNotFoundError(
            ...
            "On Streamlit Cloud, make sure `artifacts/test_predictions.csv` is uploaded "
            "to the GitHub repo at the same level as `app.py`."
        )
...
try:
    ...
    st.error("The app is deployed, but one or more required artifact files are missing.")
    st.write(str(exc))
    st.markdown(
        """
        The GitHub repository must contain this structure:
        ...
        Upload the full `artifacts` folder to GitHub, commit the change, and reboot the
        Streamlit app.
        """
    )
    st.stop()
